...
| agent-runtime | `auraclaw runtime run` | Agent Harness；只使用 Session/Control/Model/MCP/Artifact 客户端 |
| model-gateway | `auraclaw model-gateway run` | Model Provider、Secret、Quota、Usage、Fallback 信任域 |
| action-hands | `auraclaw hands run` | 托管 MCP Gateway、Hands/Sandbox、Invocation/Attempt/Side Effect |
...
- Runtime 到 Action Hands 使用 MCP revision `2026-07-28`、JSON-RPC over Streamable HTTP。
...
### 6. Action Hands 与 MCP

Agent Runtime 是 MCP Client，Action Hands 是 AuraClaw 唯一受管 MCP Server/Gateway。Runtime 的网络策略禁止直连第三方
MCP endpoint。第三方 MCP Server 只能作为 Hands registry 中的受管 Adapter 接入。

内部 MCP 默认使用 `2026-07-28` 无状态 profile，通过逐请求 `_meta` 声明版本和 Client capability，
并用可选 `server/discover` 发现 Server capability；`2025-11-25 initialize` 仅作为滚动升级兼容入口。
第一阶段支持 `tools/list`、`tools/call`、
结构化结果，并按需支持 progress、cancellation 与 Resource Link。AuraClaw 权威上下文放在 `_meta.auraclaw` 或由服务端
身份/Assignment 派生，不能信任模型可控 arguments 提供 tenant、Actor、Policy、Credential 或 fencing 字段。

JSON-RPC request id 只做协议关联；`tool_invocation_id` 是业务幂等键。Hands Invocation Store 持久保存 Invocation、Attempt、
normalized argument digest、result/artifact refs、status、lease、deadline 和 `side_effect_status`。连接断开不等于取消；重启后按
`tool_invocation_id` 查询或恢复。MCP Task/Progress 不替代该 Store、Canonical Event 或 Delivery Job。
...
### 9. Artifact 与 SeaweedFS

已部署的 SeaweedFS 作为 S3-compatible Object Store。Artifact Service 使用 PostgreSQL 保存 Artifact Metadata、ACL、Version、
Lineage、Classification、Scan State、Retention 和 Audit；SeaweedFS 只保存对象字节。生产禁用本地共享 `artifact_root`。
...
本地 `.env` 已存在 SeaweedFS 变量名。Settings 使用显式字段/alias 映射；Access/Secret Key 只注入 Artifact Service，配置值不进入
...
| Hands Invocation/Attempt | action-hands | Runtime 只经 MCP |
...
| Artifact Metadata | artifact-service | Object bytes 在 SeaweedFS |
| Delivery Job/Attempt/DLQ | delivery-worker | Ops 经 Admin API |

每个生产服务使用独立 workload identity 与最小权限数据库角色。自动化必须使用非超级用户验证越权 SQL、跨 tenant、伪造 Actor、
过期 Lease、旧 fencing token、Credential 泄漏和 Artifact ACL 均被拒绝。
...
- S3 回滚顺序为 callers 回退 → owner services 回退 → 撤销 role DSN → 执行 down migration；若已经
...
阶段门禁见 [开发阶段校验清单](./开发阶段校验清单.md) 的 S0–S5。目标实现与当前缺口映射见
