...
不要混用两套 env。生产 / DEV_SERVICE 用 `.env.production`（勿提交）。
...
```bash
# 仓库根目录
cp .env.example .env   # 若尚无 .env，按实际填库/模型/令牌
docker --version
...
- Runtime → MCP：`AURACLAW_HANDS_MCP_URL=http://action-hands:8006/mcp`（Compose 里已写死给 runtime）
...
curl -sf http://127.0.0.1:8006/health/ready   # action-hands（内部 MCP）
...
Runtime 应指向 `http://action-hands:8006/mcp`，不要把 MCP 改到 runtime 容器。
...
1. `.env.production` 已就位（0600，不进 Git）
...
2. `action-hands` healthy（内部 MCP Gateway）
3. `agent-runtime` healthy，且能连 `http://action-hands:8006/mcp`
...
| 组件 | 容器 | 职责 |
|------|------|------|
| 内部 MCP Gateway | `action-hands` | Runtime 唯一 MCP 入口：`/mcp` |
| Runtime | `agent-runtime` | 只持有 `HandsMcpClient`，不持远端 URL/Secret |
