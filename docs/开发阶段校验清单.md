...
依据：[MCP Server 热配置需求设计](./MCP%20Server%20热配置需求设计.md)；实现跟踪：
...
依据：AuraMCP 仓库 `docs/AuraClaw 接入.md`；本仓 [AuraMCP 接入](./AuraMCP%20接入.md)；
...
依据：[公开 API 与身份接入手册](./公开%20API%20与身份接入手册.md)；AuraX [GitHub Issue #1](https://github.com/sushaofei/AuraX/issues/1)。
...
依据：[公开 API 与身份接入手册](./公开%20API%20与身份接入手册.md)；[Task Query Result Service](./Managed%20Agent%20系统架构/05%20Task%20Query%20Result%20Service.md)；[GitHub Issue #52](https://github.com/sushaofei/AuraClaw/issues/52)。
...
- [x] `.kingbase.env` / `KINGBASE_*` → `DB_*` 映射（backend=kingbase 时覆盖）。
...
- [x] `.kingbase.env.example`、README 主存储章节、Issue #53 方案。
- [x] Issue #54：本地 PostgreSQL/Kafka 默认、统一应用 DSN、清理废弃 Model Skill MySQL 源配置；
  intentional commit（不含 `.env.*` / `.kingbase.env` / `.postgresql.local.env` Secret）。
