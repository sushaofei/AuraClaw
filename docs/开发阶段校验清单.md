...
- [ ] Runtime 通过一个内部 MCP Capability Gateway 发现和使用 Resource、Tool、Prompt 与 Skill。
- [ ] 建立受管 Server Registry、Capability Catalog、Skill Package 和渐进式披露。
- [ ] 外部 MCP 连接、鉴权、凭证、策略、脱敏和 Artifact 边界由平台服务管理。
- [ ] MCP 不替代 Canonical Event、Control Lease、Hands Invocation Store 或 Result Delivery。
- [ ] Runtime 不直连第三方 MCP Server，不接收 Server URL、stdio command 或明文 Secret。

### Phase 1：Catalog 与 Resource

- [ ] initialize 协商、分页 list、Catalog 同步和只读 capability search 有契约测试。
- [ ] Resource list/read 支持 tenant ACL、Policy、MIME/大小限制、DLP/注入扫描和 Artifact 化。
- [ ] Resource revision/digest 可追溯；订阅通知仅用于缓存失效。
- [ ] 现有 tools/list 与 tools/call 行为保持 N/N-1 兼容。

### Phase 2：Skill

- [ ] Skill Manifest、签名、不可变包、发布准入和撤销流程完成。
- [ ] Skill Resolver 固定版本、Tool/Resource 依赖、Policy 版本和 package digest。
- [ ] Skill Runner 支持渐进加载、预算、停止条件、Checkpoint 和崩溃恢复。
- [ ] `skill.activated/completed/failed/cancelled` 事件和 Projection/查询契约完成。

### Phase 3：远端 MCP 与安全

- [ ] Credential Proxy MCP Egress Connector 支持 OAuth/OIDC 发现和 Resource Indicator。
- [ ] Token passthrough、任意 URL/命令、SSRF、DNS rebinding 和跨 tenant 请求被拒绝。
- [ ] Server/Tool/Resource/Skill 元数据按不可信输入处理；签名 Skill 才能进入受信指令层。
- [ ] Skill 与 Tool 组合经过数据流、权限升级、互斥和外发策略检查。

### Phase 4：通知、长调用与生产门禁

- [ ] list_changed、Resource subscription 和周期对账不会造成活动绑定静默漂移。
- [ ] 若启用实验性 MCP Tasks，task id 绑定 auth/invocation 且不替代 AuraClaw 状态机。
- [ ] 多副本 Catalog/Invocation、Server 断连、响应丢失、撤销和降级恢复测试通过。
- [ ] Ruff、Mypy、Pytest、import-linter、真实 MCP 冒烟和安全测试全部通过。
- [ ] README、架构、API、迁移、运维、回滚和阶段测试报告已同步。
- [ ] 暂存范围不含 `.env`、Secret、缓存或虚拟环境；阶段以一个 intentional commit 提交并 push。
