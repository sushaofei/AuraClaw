...
- [ ] Event Store、Projection Store、Delivery Store 与技术适配器按职责拆分。
- [ ] `infrastructure/postgres.py` 拆分后目标文件均不超过 RFC 限制。
- [ ] Projection 规则不依赖具体 infrastructure；事实源、幂等、gap、checkpoint 与重建语义不变。
- [ ] 数据库 Schema/迁移确认为不涉及，或同步提供 up/down migration 与回滚说明。
- [ ] Ruff、Mypy、Pytest、PostgreSQL 契约测试与 Projection CLI 冒烟通过。
- [ ] 文档、暂存范围、intentional commit 与 push 证据已记录。

### R1.2：bounded context 分包

- [ ] `application/` 职责迁入 session、gateways、control、action、delivery 与 observability 主归属包。
- [ ] API、Canonical Event、Projection、tenant、权限、幂等、并发和敏感信息行为不变。
- [ ] 组件/包/进程入口映射及旧路径迁移表已更新。
- [ ] Ruff、Mypy、Pytest 与关键 API/Streaming 冒烟通过。
- [ ] 文档、暂存范围、intentional commit 与 push 证据已记录。

### R1.3：composition 剥离

- [ ] entrypoint → composition → api/gateways/adapters 单向装配成立。
- [ ] `api`、gateways 和业务包不导入 composition；infrastructure 不导入 api/gateways/composition。
- [ ] FastAPI dependency override、lifespan、Development Runtime、Projection/Operations CLI 行为不变。
- [ ] 真实 `auraclaw serve`、前端 Streaming/Result 一致性和现有 CLI 冒烟通过。
- [ ] Ruff、Mypy、Pytest、文档、暂存范围、intentional commit 与 push 证据已记录。

### R1.4：端口分组与依赖门禁

- [ ] session、projection、control、runtime、action、delivery ports 归属清晰。
- [ ] import-linter 使用多个小型 forbidden/independence contract，且 `uv run lint-imports` 通过。
- [ ] 唯一写入方由契约/集成测试验证，不以 import lint 代替行为验证。
- [ ] 临时旧路径 shim 已删除，仓库内旧 import 为零。
- [ ] Ruff、Mypy、Pytest 与全部关键冒烟通过；安全、迁移和回滚结论已记录。
- [ ] README、AGENTS、架构说明、Python 后端结构与运维说明已同步。
- [ ] 最终暂存范围不含本地配置、Secret、缓存或虚拟环境；intentional commit 已 push 到 `origin`。
