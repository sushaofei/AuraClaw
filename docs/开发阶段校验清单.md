...
- [ ] `contracts/` 定义 Session、Control、Model、Policy、Credential、Artifact、Admin DTO、版本和稳定错误码。
- [ ] 每个内部写入携带 tenant、service identity、command/operation id、actor、correlation、causation。
- [ ] Session append 支持 expected version、operation/event allowlist、签名 Lease Assertion 和 Fencing Token。
- [ ] Control API 支持 Runtime register/heartbeat/drain、Assignment、Lease、Checkpoint、Cancel、Validate Lease。
- [ ] Runtime↔Hands MCP 固定 2025-11-25，支持 initialize、tools/list、tools/call 和 `_meta.auraclaw`。
- [ ] Hands 使用稳定 `tool_invocation_id`，不以 JSON-RPC request id 代替业务幂等键。
- [ ] Policy/Approval、Credential Proxy、Artifact/SeaweedFS、owner Admin API 契约完整。
- [ ] in-process adapter 与 HTTP/MCP adapter 使用同一套 contract tests。

### 架构、数据与安全校验

- [ ] 业务包不 import composition；API/gateway 不选择具体基础设施适配器。
- [ ] Runtime、Hands、Delivery 不接收明文 Secret；模型/工具可控字段不能伪造权威上下文。
- [ ] Hands Invocation Store、Policy、Credential Audit、Artifact Metadata schema 与 migration/rollback 已设计并实现。
- [ ] 新旧 Adapter 可按配置回滚，公开 API 与 Canonical/Projection 语义不变。

### 质量与 Git 交付校验

- [ ] Ruff、Mypy、Pytest、import-linter 与新增 contract/permission tests 通过。
- [ ] 暂存范围不含 `.env`、Secret、用户无关改动、缓存或虚拟环境。
- [ ] S1 作为一个 intentional commit 提交并 push 到 `origin`。
...
- [ ] `api/session/projection/orchestrator/runtime/model-gateway/hands/policy/credential-proxy/artifact/streaming/delivery` 入口可独立启动。
- [ ] 每个入口只装配自身对象图、配置和 Worker，不隐式启动其他生产服务。
- [ ] `auraclaw serve` 明确为 development combined profile。
- [ ] Docker Compose/Ingress 可一键启动 12 进程，并支持外部 SeaweedFS endpoint。
- [ ] 每服务具有 live/ready、SIGTERM 优雅停机、硬依赖 readiness 与配置失败语义。
- [ ] `.env.example` 只含无密钥占位符；SeaweedFS/Vault/DB/MCP 配置日志不泄漏值。

### 质量与 Git 交付校验

- [ ] 12 入口 CLI、live/ready、信号与配置 contract tests 通过。
- [ ] Ruff、Mypy、Pytest、import-linter 与 Compose smoke 通过。
- [ ] S2 作为一个 intentional commit 提交并 push 到 `origin`。
...
- [ ] Task API、Runtime、Orchestrator、Policy、Hands、Delivery 全部通过 Session Client 追加 Canonical Event。
- [ ] 非 Session 服务没有 Event Store 写凭证或 direct append 装配。
- [ ] Orchestrator 是 Control Store 唯一写入者；Runtime 只使用 Control Client。
- [ ] Projection/Delivery 通过 Session Outbox claim/feed 消费并 ack，不直接更新 Session schema。
- [ ] Model Provider/Secret 迁入 Model Gateway；Runtime 仅保留 Model Client。
- [ ] Tool/Sandbox/Invocation Store 迁入 Action Hands；Runtime 仅保留 MCP Client。
- [ ] Policy Engine/Approval control 迁入 Policy；Credential/Vault adapter 迁入 Credential Proxy。
- [ ] Artifact Metadata/API 迁入 Artifact Service，SeaweedFS S3 adapter 接线；生产禁用本地 Artifact 目录。
- [ ] Operations CLI/Task Ops 只调用 owner Admin API，不跨 schema redrive/retention。

### 权限、安全与迁移校验

- [ ] 独立 DB roles/schema grants、`task_query_ro`、workload identity 与网络策略已落地。
- [ ] 越权 SQL、跨 tenant、伪造 actor、旧 fencing、过期 approval、scope 越权、Artifact ACL 均被拒绝。
- [ ] Runtime 不能直连第三方 MCP，Hands/Delivery 不能读取 Vault Secret，Artifact 消费者没有 SeaweedFS 管理凭证。
- [ ] 数据迁移、双读/双写窗口、回滚和 N/N-1 兼容测试通过。

### 质量与 Git 交付校验

- [ ] Ruff、Mypy、Pytest、import-linter、PostgreSQL grants、SeaweedFS/Vault 集成测试通过。
- [ ] S3 作为一个 intentional commit 提交并 push 到 `origin`。
...
- [ ] Orchestrator 2+ 副本 claim/lease/reconcile 互斥，过期 Assignment 可恢复。
- [ ] Agent Runtime 2+ 副本无黏性接管，Checkpoint/Session 恢复且旧 fencing 失效。
- [ ] 生产调度不扫描完整 Event Log，Runnable Outbox/Feed 按 source version 幂等驱动。
- [ ] Hands MCP Gateway/Executor 2+ 副本使用持久 Invocation Store，连接断开不丢调用事实。
- [ ] Projection/Delivery 多 Worker 保持同 Session 顺序、幂等、Retry/DLQ/Manual Redelivery。
- [ ] Streaming 使用共享 Connection Registry/Replay Router，handoff/restart/rebalance 后 cursor 连续。
- [ ] Policy/Artifact/Model/Credential 多副本共享版本、配额、撤销、扫描和审计状态。
- [ ] SeaweedFS multipart/finalize/scan/GC 与短暂故障恢复通过。

### 质量与 Git 交付校验

- [ ] 所有声明可横向扩展的服务完成 2+ 副本集成与压力基线测试。
- [ ] Ruff、Mypy、Pytest、import-linter 与恢复测试通过。
- [ ] S4 作为一个 intentional commit 提交并 push 到 `origin`。
...
- [ ] 生产模板明确 replica、resource、HPA/PDB、service identity、DB role、NetworkPolicy 和 Secret mount。
- [ ] N/N-1 内部契约、expand/migrate/contract 数据库迁移与滚动升级通过。
- [ ] 12 服务 E2E：任务 → Runnable → Assignment → Model/MCP → SSE → Canonical Result → Delivery。
- [ ] Kill tests 覆盖 Session、Runtime、Orchestrator、Model、Hands、Policy、Credential、Artifact、Streaming、Delivery。
- [ ] PostgreSQL、Kafka、Replay Router、SeaweedFS、Vault 短暂不可用的背压、fail-closed、恢复和告警通过。
- [ ] Ops E2E 覆盖 Projection rebuild/redrive、Delivery redrive/DLQ、Retention 与审计。
- [ ] Runbook、架构总览、代码映射、配置、迁移、回滚和扩缩容说明已同步。

### 最终质量与 Git 交付校验

- [ ] Ruff、Mypy、Pytest、import-linter、Compose/部署 smoke、权限与故障演练全部通过。
- [ ] 最终暂存范围不含 `.env`、Secret、用户无关改动、缓存或虚拟环境。
- [ ] S5 作为一个 intentional commit 提交并 push 到 `origin`；Issue #12 可关闭。
